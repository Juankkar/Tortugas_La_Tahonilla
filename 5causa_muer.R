...
gfc_anova <- dfc_ult.decada %>% 
  ...
  labs(title = "Comparación de las causas 2013-21",
       subtitle = "X\u00B2 Kruskal-Wallis = 4.8; PV = 0.09",
        x = "Causas",
        y = "Núm.tortugas") +
...
kruskal.test(n~causa, data = dfc_2000_2011)                                     
...
gfc_kw <- dfc_2000_2011 %>% 
  ...
  labs(title = "Comparación de las causas 2001-12",
       subtitle = "X\u00B2 Kruskal-Wallis = 21.52; PV = 2.12e-05***\nPost-hoc: Test de Dunnet (corrección: Bonf.)",
       x = "Causas",
       y = "Núm.tortugas") +
  theme_classic() +
  theme(plot.title = element_markdown(size = 11.5, face = "bold", hjust = .5),
        plot.subtitle  =element_text(size = 10.5, hjust = .5),
        panel.background = element_blank(),
        axis.line.y = element_line(),
        axis.ticks.x = element_blank(),
...
wilcox.test(n~factor(muerte), data = anio_muerte)
...
rehab_plis <- expand.grid(rehabilitadas=as.character(unique(as.character(r_nor$rehabilitadas))),
                          anio=as.character(unique(as.character(r_nor$anio)))) %>% left_join(
                             ...
                             ) %>% mutate(n=ifelse(is.na(n),0,n)) %>% arrange(anio) %>%  view()

tapply(rehab_plis$n, rehab_plis$rehabilitadas, shapiro.test)
wilcox.test(n~as.factor(rehabilitadas), data = rehab_plis)
