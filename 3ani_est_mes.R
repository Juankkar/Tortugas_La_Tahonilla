...
gfe_bar_n <- tor_anio_mes %>% 
  ...
  labs(
    title = "Número de tortugas por Año",
    x = "Años",
    y = "Núm.tortugas",
    fill = NULL) +
...
## Estdio de la normalidad y homocedasticidad

tapply(tor_anio_est$n, tor_anio_est$estacion, shapiro.test)          # p > 0.05
LeveneTest(tor_anio_est$n, tor_anio_est$estacion, center = mean)     # p > 0.05
...
### Kruskal-Wallis
library(FSA)
KWE <- kruskal.test(n~estacion, data = tor_anio_est)
KWE$statistic
KWE$p.value
...
gfe_kw <- tor_anio_est %>%
  ...
  labs(title = "Llegada al CRFS según la estación",
       subtitle = "X\u00B2 Kruskal-Wallis = 56.56, P = 3.19e-12***\nPost-hoc: Test de Dunnet (corrección: Bonf.)",
       y = "Núm. tortugas",
       x = "Estación del año",
       fill=NULL) +
  theme_classic() +
