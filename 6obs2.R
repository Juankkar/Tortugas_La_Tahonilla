...
df_tortugas_obs <- df_tortugas2 %>% mutate(
...
graf_lesion <- lesion %>% 
  ...
  geom_text(data = etiquetas_lesiones,
            aes(x=x,y=y,label=n_character),inherit.aes = F,size=3.5) +
...
graf_cuerpo <- cuerpo %>% 
  ...
  geom_text(data = etiquetas_cuerpo,
            aes(x=x,y=y,label=n_character),inherit.aes = F, size=3.25) +
...
fila_2 <- plot_grid(map_calor1,"",
                    ncol = 2, nrow = 1,
                    labels = c("B",""),
                    rel_widths = c(.75,.25));fila_2
