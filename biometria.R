...
biometria <- read_excel("C:/Users/jcge9/Downloads/data_tortugas.xlsx", 
  ...
  mutate(anio=as.character(format(fecha_ok, "%Y")),
         ...
         estacion=case_when(mes == "dic."~"Invierno",
                            ...
                            mes == "oct."~"Otoño",
                            mes == "nov."~"Otoño")) %>% 
  filter(l.recto > 0,l.curvo > 0,
       a.curvo > 0,a.recto > 0,
      peso > 0,peso < 250) %>% 
  mutate(estadio=case_when(l.curvo<20 ~ "Cría",
                         l.curvo>=20 & l.curvo<40~ "Juv.pequeño",
                         l.curvo>=40 & l.curvo<60~"Juv.grande",
                         l.curvo>=60 & l.curvo<80 ~"Subadulto",
                         l.curvo>=80 ~"Adulto")) %>% view()

 # biometria %>%
 #   group_by(especie,estadio) %>%
 #   summarise(n=n())
 #   summarise(n=n(),
 #            media_l.recto=mean(l.recto),
 #            media_l.curvo=mean(l.curvo),
 #            media_a.recto=mean(a.recto),
 #            media_a.curvo=mean(a.curvo),
...
biometcm <- biometria %>% filter(especie %in% "Caretta caretta")
...
# #------------------------------------------------------------------------------#
# 
biomet.cc <- biometria %>% filter(especie %in% "Chelonia mydas")
        # n = 18
...
variables <- biomet.cc[,c("l.recto","l.curvo","a.recto","a.curvo","peso")] 
...
cor(biomet.cc[,c("l.recto","l.curvo","a.recto","a.curvo","peso")],
    pca_biomet[,c("PC1","PC2")])
...
library(psych)
pairs.panels(biomet.cc[,c("l.recto","l.curvo","a.recto","a.curvo","peso")],
             main = "Correlacion libeal tortuga boba")
